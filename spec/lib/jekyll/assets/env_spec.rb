...
describe Jekyll::Assets::Env do
  ...
  context "digesting" do
    context "in production" do
      it "digests by default" do
        ...
        expect(uncached_env.send(:as_path, uncached_env.find_asset( \
          "bundle.css"))).to match %r!bundle-([a-zA-Z0-9]+)\.css\Z!
      end
  ...
  context "compression" do
    it "compresses if asked to" do
      ...
      expect(uncached_env.css_compressor).to eq \
    ...
    context "in production" do
      ...
      it "disables compression if asked to" do
        allow(Jekyll).to receive(:env).and_return "production"
        stub_asset_config uncached_site, "compress" => { "js" => false, "css" => false }
  ...
  context do
    ...
    before(:each) { uncached_site.process }
