...
describe "Magick AssetPath Proxy" do
  ...
  def stub_tag(*args)
    Jekyll::Assets::Liquid::Tag.send(:new, "asset_path", "ubuntu.png #{args.join(" ")}", []).render(
  ...
  def get_asset(html)
    Pathutil.new(@env.in_cache_dir(html.gsub(
  ...
  context "modified files formats" do
    def create_asset
      tag = Jekyll::Assets::Liquid::Tag.send(:new, "asset_path", "ubuntu.png magick:format:jpg", [])
